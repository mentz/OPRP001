...
__global__ matrix_sum_kernel(double *a, double *b, double *ret) {
...
matrix_t *matrix_sum_gpu(matrix_t *A, matrix_t *B, matrix_t *ret) {
  ...
  int mat_size = sizeof(double)*newRows*newCols;

  cudaMalloc(d_a, mat_size);
  cudaMalloc(d_b, mat_size);
  cudaMalloc(d_ret, mat_size);

  cudaMemcpy(d_a, A->data[0], mat_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_b, B->data[0], mat_size, cudaMemcpyHostToDevice);
